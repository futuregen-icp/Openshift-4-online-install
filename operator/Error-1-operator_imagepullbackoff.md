...
## 특이사항
- 모든 노드가 롤아웃 되면서 , 파드가 리스케줄 되는 과정이 진행이 되므로, 영향도가 높다.
- 또한 모든 노드가 롤아웃 되고, 다시 롤인 되면서 글로벌 클러스터 시크릿이 업데이트가 되어야 하는데, 적용 되지 않는 사례가 확인 되있다.
- 이 작업 이후에는 각 노드에서 풀시크릿이 업데이트가 되었는지 검증해야 할 것이다. 
```
crictl pull registry.oss2.fu.igotit.co.kr:5000/olm/redhat-operators:v1
```
