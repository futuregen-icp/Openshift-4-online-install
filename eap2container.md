...
그리고 다음에 그쪽에서 진행하고, 또 요구사항 확인하고 일정을 확인해서 왔다갔다 하면서 진행하자!
...
지금과 같이 소통의 문제가 있고, 그리고 또 테스트해야 하는 업무가 있고, 업무의 분량이 크며, 
테스크 정의해야 할 것 같습니다.
첫번째로 상주해서 레거시 시스템에서 컨테이너 전환을 위해서 컨테이너 전환 테스트를 수행하고 잘 안되는 부분을 cowork 해서 기본이미지를 만들어 가야하는데,
여기에서 문제점은 bxm엔지니어가 assign 되어야 합니다.
```
