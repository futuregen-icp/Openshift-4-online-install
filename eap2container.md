...
bxm 제공하는 환경을 구성하는 가이드를 모두 검토하고 기본이미지를 만들어서 base image(template) 환경은 제공하고 일정을 맞춰서 진행하자!

개인 견해로는 상주해서 컨테이너 빌드테스트를 뱅크웨어글로벌 직원이 필요 하고, 지금 이미지를 테스트하고
잘 안되는 부분을 cowork 해서 기본이미지를 만들어 가야 함

기본이미지 만들 때
+ 기존의 스크립트 환경의 절차 스크립트 확인 (1day)
+ bxm 가이드문서 분석 및 이미지에 반영 (?)
